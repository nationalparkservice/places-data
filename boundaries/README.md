...
- Enter your password (Jim set these up. Hint: Wm4U!)
...
1. Update the `parks` table's `unit_area` field, along with the `data_source` if this has changed and `geom_point` if applicable (continuing MUWO example):
 - `psql -U postgres -d places_boundaries -c "UPDATE parks SET unit_area = (SELECT ST_Area(geom_poly) FROM parks_poly WHERE unit_code = 'muwo'), data_source = 'Land Resources Division' WHERE unit_code = 'muwo';"`
